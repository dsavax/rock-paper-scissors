...
module.exports = (config) => {
   ...
   */
  function create(requestId, player1id) {
    return httpClient(requestId,
    {
   ...
   */
  function fetch(requestId, criteria) {
    return httpClient(requestId,
      {
        uri: `${config.protocol}://${config.host}:${config.port}/api/v1/games`,
        method: 'GET',
   ...
   */
  function update(requestId, id, body) {
    return httpClient(requestId,
      {
        uri: `${config.protocol}://${config.host}:${config.port}/api/v1/games/${id}`,
        method: 'PATCH',
